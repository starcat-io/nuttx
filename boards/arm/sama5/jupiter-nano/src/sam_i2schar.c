...
#ifndef CONFIG_JUPITER_NANO_SSC_PORT
#  if defined(CONFIG_SAMA5_SSC0)
#    define CONFIG_JUPITER_NANO_SSC_PORT 0
#  elif defined(CONFIG_SAMA5_SSC1)
#    define CONFIG_JUPITER_NANO_SSC_PORT 1
#  endif
#endif

#ifndef CONFIG_JUPITER_NANO_I2SCHAR_MINOR
#  define CONFIG_JUPITER_NANO_I2SCHAR_MINOR 0
#endif
  ...
  if (!initialized)
    {
      ...
      i2s = sam_ssc_initialize(CONFIG_JUPITER_NANO_SSC_PORT);
      if (!i2s)
        {
          _err("ERROR: Failed to get the SAMA5 SSC/I2S driver for SSC%d\n",
              CONFIG_JUPITER_NANO_SSC_PORT);
          return -ENODEV;
        }

      /* Register the I2S character driver at "/dev/i2schar0" */

      ret = i2schar_register(i2s, CONFIG_JUPITER_NANO_I2SCHAR_MINOR);
      if (ret < 0)
        {
